...
def a_star_search(graph, start_node, goal_node):
    ...
    while node != goal_node:
        ...
        for neighbour in node_neighbours: # Iterates through the neighbours of the current node
            ...
            if neighbour == goal_node: # if the neighbour ends up being the goal node
                route.append(node+goal_node) # update the route list
                total_cost += graph.get_cost(node,goal_node) # update cost
               
               # print node, cost and heuristic
                print(f"{node+goal_node} --> Cost({graph.get_cost(node,goal_node)}) + Heuristic({graph.get_h(goal_node)})",
                  f" =  {graph.get_cost(node,goal_node)+graph.get_h(goal_node)}"
            )
                ...
                end_loop = 1 # To end the outside while loop
                break
                
            else: # If the neighbour is not the goal node. It just updates the dict with node and its f(n)
                f = graph.get_h(neighbour) + graph.get_cost(node,neighbour)
                store_node_vals[node+neighbour] = f


        if end_loop == 1: # Breaks the outside while loop
            break
        ...
        else: # If the f(n) of 2 nodes is different, we simple take the one which is minimum
